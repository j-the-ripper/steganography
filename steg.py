...
while True:
    ...
    if m == "1":
        ...
        #svd_img_encoded = SVD().svdEncode(img, msg)
        ...
        #svd_encoded_image_file = "svd_" + original_image_file
        ImageResult = mp.svdEncode(img, msg)
        cv2.imwrite('../Encoded_image/svdResult.png', ImageResult)
        ...
        svd_hidden_text = DCT().decode_image(dct_img)
        ...
        file.close()
        file = open("svd_hidden_text.txt", "w")
        file.write(svd_hidden_text)  # saving hidden text as text file
        file.close()
        ...
        #svdEncoded = cv2.imread('svdResult.png')
        ...
        #svd_encoded_img = cv2.cvtColor(ImageResult, cv2.COLOR_BGR2RGB)
        #        dwt_encoded_img = cv2.cvtColor(dwtEncoded, cv2.COLOR_BGR2RGB)
        os.chdir("..")
        os.chdir("Comparison_result/")
        # bImg, gImg, rImg = cv2.split(original)
        # bImg = np.float32(bImg)
        book = xlwt.Workbook()
        ...
        sheet1.write(2, 1, Compare().meanSquareError(original, dct_encoded_img)/1000)
        sheet1.write(2, 2, Compare().psnr(original, dct_encoded_img))
        sheet1.write(3, 0, "SVD")
        sheet1.write(3, 1, Compare().meanSquareError(original, ImageResult))
        sheet1.write(3, 2, Compare().psnr(original, ImageResult))
